...
export async function GET(req: NextRequest) {
  try {
    ...
    const countRow = await dbFirst<{ total: number }>(
      "select count(1) as total from trade_submissions where user_id = ? and type = ? and created_at >= ?",
      [user.id, "trade_log", cutoff]
    );

    const submissions = await dbAll(
      "select id,status,rejection_reason,review_note,created_at from trade_submissions where user_id = ? and type = ? and created_at >= ? order by created_at desc limit ? offset ?",
      [user.id, "trade_log", cutoff, pageSize, from]
    );
