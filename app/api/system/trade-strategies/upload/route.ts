...
async function notifyAdmins(user: {
  ...
  const rows = targets.map((id) => ({
    ...
    params: [
      id,
      user.id,
      "模拟交易策略提交 / Simulation trade strategy submitted",
      `学员 ${label} 已提交模拟交易策略。\n\nStudent ${label} submitted simulation trade strategies.`,
      now
    ]
  }));
  await dbBatch(rows);
...
export async function POST(req: Request) {
  try {
      ...
      if (replace) {
        ...
        try {
          await dbRun(
            ...
            [submissionId, user.id, safeLeaderId, "trade_strategy", "submitted", now, now]
          );
        } catch (err) {
          const existing = await dbFirst<any>("select id,user_id,type from trade_submissions where id = ? limit 1", [submissionId]);
          if (!existing?.id || existing.user_id !== user.id || existing.type !== "trade_strategy") throw err;
        }
      }
    ...
    const submissionId = requestId ? buildRequestScopedId("tradestrategy", user.id, requestId) : createD1TextId();
